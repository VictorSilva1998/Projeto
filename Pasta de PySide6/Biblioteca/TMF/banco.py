...
MENSAGENS = {"AccountNotFound": "E-mail ou senha incorretos.", "InvalidEmailOrPassword": "E-mail ou senha incorretos.",
...
def _ler_saldo(playfab_id):
    dados = _post(
        "/Server/GetUserData", {"PlayFabId": playfab_id, "Keys": ["saldo"],}, secreto=True,)
...
def _gravar_saldo(playfab_id, valor):
    _post(
        "/Server/UpdateUserData", {"PlayFabId": playfab_id, "Data": {"saldo": f"{valor:.2f}"}, }, secreto=True,)
...
def cadastrar(email, nome, senha):
    dados = _post(
        "/Client/RegisterPlayFabUser", {"TitleId": config.TITLE_ID, "Email": email, "Password": senha, "DisplayName": nome, "RequireBothUsernameAndEmail": False, },)
