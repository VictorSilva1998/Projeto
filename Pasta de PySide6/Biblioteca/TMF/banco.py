...
def _post (caminho, corpo, secreto = False):
    ...
    try:
        ...
        raise ErroBanco ("Não foi possivel falar com o PlayFab")
    except ValueError:
        raise ErroBanco ("PlayFab Respondeu Doidão")
...
def _ler_saldo (playbab_id):
    dados = _post ("/Server/GetUserData", {"PlayFabId": playbab_id, "Keys": ["saldo"],}, secreto= True)
    registro = dados.get ("Data", {}).get ("saldo")
    return float (registro ["Value"]) if registro else 0.0

def _gravar_saldo (playfab_id, valor):
    _post ("/Server/UpdateUserData", {"PlayfabId": playfab_id, "Data": {"saldo": f"{valor:.2f}"}}, secreto = True)
